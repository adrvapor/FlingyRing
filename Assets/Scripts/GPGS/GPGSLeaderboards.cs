...
public class GPGSLeaderboards : MonoBehaviour
{
    ...
    public static void UpdateHighScoreLeaderboard()
    {
        ...
        if (PlayerPrefs.GetInt(UserData.HighScoreKey, 0) == 0)
            return;

        Social.ReportScore(PlayerPrefs.GetInt(UserData.TotalScoreKey, 1), GPGSIds.leaderboard_total_score, (bool success) =>
        {

        });

        Social.ReportScore(PlayerPrefs.GetInt(UserData.HighScoreKey, 1), GPGSIds.leaderboard_high_score, (bool success) =>
        {
            // Si se ha podido actualizar la puntuación en la leaderboard, cargarse el valor de PlayerPrefs
            if (success)
                PlayerPrefs.SetInt(UserData.HighScoreKey, 0);
        });
    }
}
